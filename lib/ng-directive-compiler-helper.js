...
function createCompiler(template, $rootScope, $compile) {
  ...
  return function(/*parentScope, attrs, callback*/) {
    ...
    function andGet(elementTag) {
      var childElement = element.find(elementTag).children(0);
      var childScope = childElement.scope();
      return  { scope: childScope, element: childElement};
    }

    return { scope: extendedScope.$$childHead, element: element, andGet: andGet};
  };
}
